...
###################### METHOD 1: FILTER BY ANTI COR<-.35 ######################
...
########### fix anticorrelation matrix
cor_results_long<-melt(cor_results, varnames = c('target_ensembl', 'mature_mirna_id'), value.name = 'cor' )

# could filter here
### filter using binary threhsold 
#cor_results_long_ints<-cor_results_long[cor_results_long$cor<-0.35,]
...
ggsave(paste0(mir_results_file_anticor, '_barplot',  '.jpeg'), width=8, height=7)
