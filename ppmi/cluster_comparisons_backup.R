...
## 3. enrichment 
## 4. 
se_clusters<-se_filt_V08; visit_se='V08'
se_clusters<-se_filt_BL;visit_se='BL'


outdir_clustering_enrich<-paste0(outdir, '/clustering/deseq/')
dir.create(outdir_clustering_enrich)
#visit_se='BL'
deseq_all_groups <- vector("list", length = 3)


se_clusters$kmeans_grouping<- groups_from_mofa_factors(se_clusters$PATNO, MOFAobject_clusts = MOFAobjectPD, 
                                                       y_clust='NP2PTOT' )
...
table(colData(se_clusters)[, c('kmeans_grouping', 'age_cat')])
table(colData(se_clusters)[, c('kmeans_grouping', 'SEX')])
...
deseq_sig3<-deseq2ResDF3[deseq2ResDF3$mofa_sign %in% 'Significant',]; cluster_id=3; 
rownames(deseq_sig3)<-gsub('\\..*', '',rownames(deseq_sig3))
deseq_sig3$symbol<-get_symbols_vector(rownames(deseq_sig3))
write.csv(deseq_sig3, paste0(outdir_clustering_enrich, '_clust', cluster_id, '.csv'))
deseq_sig1<-deseq2ResDF1[deseq2ResDF1$mofa_sign %in% 'Significant',]; cluster_id=1
rownames(deseq_sig1)<-gsub('\\..*', '',rownames(deseq_sig1))
deseq_sig1$symbol<-get_symbols_vector(rownames(deseq_sig1))
write.csv(deseq_sig1, paste0(outdir_clustering_enrich, '_clust', cluster_id, '.csv'))
...
dir.create(paste0(outdir, '/clustering/enrichment/', visit_se))
results_file_cluster=paste0(outdir, '/clustering/enrichment/', visit_se,'/gseGO',add_med, '_', ONT, '_', order_by_metric, 'clust', cluster_id)
gse1<-run_enrich_per_cluster(deseq2ResDF1, results_file_cluster,N_DOT=20, N_EMAP=50 )

gene_list_ora1 = deseq_sig1[, order_by_metric]
names(gene_list_ora1)<-gsub('\\..*', '',rownames(deseq_sig1))
N_EMAP=50
dir.create(paste0(results_file_cluster, '/ora/'))
run_ora_gene_list(gene_list_ora1, results_file_ora = paste0(results_file_cluster, '/ora/'), keyType = 'ENSEMBL')
...
gene_list_ora3 = deseq_sig3[, order_by_metric]
names(gene_list_ora3)<-gsub('\\..*', '',rownames(deseq_sig3))

names(gene_list3)<-gsub('\\..*', '',names(gene_list3))

run_ora_gene_list(gene_list_ora3[1:50], results_file_ora = paste0(results_file_cluster, '/ora/'), keyType = 'ENSEMBL')
...
prefix='rna'
N_EMAP=30
enrich_compare=paste0(outdir_clustering_enrich, 'compare')
