...
if (load_all_times){
  ...
  rnas_sig_BL_list<-lapply(rnas_BL_list, function(df){ return(df%>% dplyr::filter(mofa_sign == 'Significant'))})
  ...
  mirnas_sig_BL_list<-lapply(mirnas_BL_list, function(df){  return(df%>% dplyr::filter(mofa_sign == 'Significant'))})
