...
data = list(mRNA = t(highly_variable_genes_voom), 
            proteomics = t(highly_variable_genes_voom) )
...
for(k in 1:5){
  ...
  save(cv.fit, file=paste(settings,"cv.fit.k_new_fixed_",n.lambda,k,".Rdata",sep=""))
}
...
output=alist()
files=grep( paste0("cv.fit.k_new_fixed_",n.lambda),dir(settings))
...
png(paste0(icluster_out,param_str_icl,k, 'heatmap_multi_trained.png'))
