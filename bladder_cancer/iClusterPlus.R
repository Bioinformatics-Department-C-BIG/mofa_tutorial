...
data = list(mRNA = t(highly_variable_genes), 
            proteomics = t(highly_variable_proteins) )
...
for(k in 1:3){
  ...
  save(cv.fit, file=paste(settings,"cv.fit.k_new_",n.lambda,k,".Rdata",sep=""))
}
...
output=alist()
files=grep("cv.fit",dir(settings))
