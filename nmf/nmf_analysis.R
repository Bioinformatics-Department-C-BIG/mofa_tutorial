...
plot_covars<-function(cor, covars_to_plot ,fname, plot='r'){
  if (plot=="r") {
...
get_cluster_assoc<-function(df1, cluster_col){
...
get_cluster_mi<-function(df1, cluster_col){
  all_tests<-apply(df1[,to_test],2,  MutInf, y=as.factor(df1[,cluster_col] ))
  return(all_tests)
}

all_tests_m<-get_cluster_assoc(df1, 'cluster_m') 
all_tests_s<-get_cluster_assoc(df1, 'cluster_s') 
all_tests_moca<-get_cluster_assoc(df1, 'clusters_mofa_moca') 

all_tests_m_mi<-get_cluster_mi(df1, 'cluster_m') 
all_tests_s_mi<-get_cluster_mi(df1, 'cluster_s') 
...
pheatmap(as.matrix(-log10(all_tests_compare)), cluster_rows = FALSE)
