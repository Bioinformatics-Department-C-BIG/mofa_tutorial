...
if (run_heatmap){
 ...
 # detach('ComplexHeatmap',unload=TRUE)
  ...
  colDataToPlot<-c('NP1RTOT','NP2_TOT', 'rigidity', 'td_pigd_old_on', 'MCATOT' , 'RBD_TOT', 'NP3_TOT')
  df<-as.data.frame(colData(vsd_filt)[,c("COHORT", "SEX", 'AGE', 'NHY', colDataToPlot)])
  colData(vsd_filt_genes)$RBD_TOT
  # if clusters exist 
  
  df$cluster<-factor(clusters$cluster[match(colData(vsd_filt_genes)$PATNO_EVENT_ID, names(clusters$cluster ))])
