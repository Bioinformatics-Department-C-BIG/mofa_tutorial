...
preprocess_raw_data<-function(df, most_var, cut_n, ng=0.75){
...
most_var=TRUE
ng=0.75
X1_t<-preprocess_raw_data(X1_t_raw,most_var=most_var, cut_n=27000, ng)
X2_t<-preprocess_raw_data(X2_t_raw,most_var=most_var,cut_n = FALSE, ng)
...
ncomp_p=5
ng=0.75
param_str_g<-paste0( '_', most_var, '_', ncomp_g, '_ng_', ng)
param_str_g_plot<-paste( 'most var = ', most_var, ', ng =', ng,  ', ncomp = ', ncomp_g)
...
param_str_p<-paste0( '_', most_var, '_', ncomp_p, '_ng_', ng)
param_str_p_plot<-paste( 'most var = ', most_var, ', ng =', ng,  ', ncomp = ', ncomp_g)
...
cim(spca.result, xlab = "proteins", ylab = "Samples",
    save='png',name.save =paste0(output,'cim_proteins', param_str_p),
...
###### corelation plot
comp=1
png(paste0(output,'/PLS_cim',  props, '_comp_', as.character(comp), '.png'))
cim(final.spls.bladder, comp = comp, xlab = "proteins", ylab = "genes",
    row.cex=1.5, col.cex = 1.5, margins=c(8,8))
dev.off()

 
selectVar(final.spls.bladder, comp = 1)$X
